...
- **模型中心**：LLM 与向量化模型统一管理（向量模型信息与配置已并入模型中心），连接测试与用量一览。
- **AList 网盘连接器**：浏览/导入 AList 文件到知识库，支持工作目录（如 /115/璇玑）限定账号可见范围。
- **云备份**：知识库备份到本地/NAS/AList（REST 协议），目标目录不存在时自动递归创建。
- **天宫联动**：作为天宫多 Agent 平台的知识底座，接收任务完成记忆、提供上下文检索。
...
设置 → 连接器 → AList，填写：URL（站点地址）、用户名、密码、**工作目录**（如 `/115/璇玑`，留空为账号根目录）。

- 浏览顶层即工作目录；点「返回上级」越过工作目录时自动弹回，不会报权限错
- 「导入」会把文件拉入知识库，文本类自动分块向量化
- 连接测试会透传 AList 服务端真实报错（如 permission denied / object not found）

### AList 备份仓库（REST 协议）

备份目标的 URL 直接带子目录作为备份落点：`https://your-alist/115/璇玑`（旧的 `…/dav/115/璇玑` WebDAV 写法也兼容，自动剥掉 `/dav`）。

- 目标目录不存在时**自动递归创建**（测试连接时也会顺手建出来）
- 前提：账号对父级目录有写权限
- 备份内容加密存储需配置 `BACKUP_ENCRYPTION_KEY`

#### 版本化快照与保留策略

每次备份运行都会在备份落点下新建一个 **UTC 时间戳快照目录**（如 `2026-09-17T02-00-00/`），
该次运行的 `manifest.json` 与全部文件都落在其中；恢复时按作业记录的 `remoteDir` 回到同一目录。

- 每个快照都是**完整备份**，占用 = 单份体积 × 保留份数（例如单份 1.2 GB 时，留 3 份 ≈ 3.5 GB）
- 保留份数由计划的「保留最近 N 份」控制，备份成功后自动清理更旧的快照
- 清理需要 AList 账号具备 **Delete 权限位**（角色权限里的 Delete 开关，
  权限值加 128）；权限不足时清理只记日志告警，**不影响备份结果**
- 该能力由仓库实现的 `supportsRunDirs` / `pruneRuns` 提供（目前 AList 仓库支持）

### 天宫联动

在设置 → 连接器中配置天宫（Tiangong）地址与 API Key 后：

- 天宫任务完成记忆自动写入璇玑知识库（带来源 traceId，可溯源）
- 天宫侧可经 `searchContext` 检索璇玑知识作为任务上下文
