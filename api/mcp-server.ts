...
async function apiKeyUser(headers: Headers): Promise<User | undefined> {
  const token = bearer(headers);
  if (!token) return undefined;
  const tokenHash = createHash("sha256").update(token).digest("hex");
  const db = getDb();
  const keyColumns = [
    { column: "ak.key_hash", value: tokenHash },
    { column: "ak.keyHash", value: tokenHash },
    { column: "ak.token", value: token },
    { column: "ak.key", value: token },
  ];
  const userColumns = ["ak.user_id", "ak.userId", "ak.createdBy"];
  const activeFilters = [
    sql`(ak.expires_at IS NULL OR ak.expires_at > NOW()) AND ak.revoked_at IS NULL`,
    sql`(ak.expiresAt IS NULL OR ak.expiresAt > NOW()) AND ak.revokedAt IS NULL`,
    undefined,
  ];
  for (const keyColumn of keyColumns) {
    for (const userColumn of userColumns) {
      for (const activeFilter of activeFilters) {
        try {
          const rows = await db.select({ id: users.id, unionId: users.unionId, name: users.name, email: users.email, avatar: users.avatar, role: users.role, createdAt: users.createdAt, updatedAt: users.updatedAt, lastSignInAt: users.lastSignInAt })
            .from(sql`api_keys ak`)
            .innerJoin(users, eq(users.id, sql.raw(userColumn)))
            .where(and(eq(sql.raw(keyColumn.column), keyColumn.value), activeFilter))
            .limit(1);
          if (rows[0]) return rows[0];
        } catch (caught) {
          if (caught instanceof Error) continue;
          throw caught;
        }
      }
    }
...
async function authenticate(headers: Headers): Promise<User | undefined> {
  return (await apiKeyUser(headers)) ?? authenticateLocalRequest(headers);
}
...
async function handleKnowledgeCreate(args: Record<string, unknown>, user: User): Promise<McpToolResult> {
...
async function handleDocumentRead(args: Record<string, unknown>): Promise<McpToolResult> {
...
async function handleDocumentWrite(args: Record<string, unknown>, user: User): Promise<McpToolResult> {
...
async function handleBackupList(args: Record<string, unknown>): Promise<McpToolResult> {
...
async function handleBackupTrigger(args: Record<string, unknown>): Promise<McpToolResult> {
...
async function handleWorkflowList(args: Record<string, unknown>): Promise<McpToolResult> {
...
async function handleWorkflowExecute(args: Record<string, unknown>, user: User): Promise<McpToolResult> {
...
async function callTool(call: McpToolCall, user: User): Promise<McpToolResult> {
  switch (call.name) {
    case "knowledge_search": return handleKnowledgeSearch(call.arguments);
    case "knowledge_create": return handleKnowledgeCreate(call.arguments, user);
    case "document_read": return handleDocumentRead(call.arguments);
    case "document_write": return handleDocumentWrite(call.arguments, user);
    case "backup_list": return handleBackupList(call.arguments);
    case "backup_trigger": return handleBackupTrigger(call.arguments);
    case "workflow_list": return handleWorkflowList(call.arguments);
    case "workflow_execute": return handleWorkflowExecute(call.arguments, user);
...
export async function handleMcpRequest(body: unknown, headers: Headers): Promise<JsonRpcResponse> {
  ...
  if (!user) return err(request.id, -32001, "Authentication required");
  ...
  try {
    ...
    return ok(request.id, await callTool(call, user));
  } catch (caught) {
