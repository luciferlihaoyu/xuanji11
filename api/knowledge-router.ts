...
export const knowledgeRouter = createRouter({
  ...
  autoLinkEdges: adminQuery
    ...
    .mutation(async ({ input, ctx }) => {
      const { autoLinkEdgesCore } = await import("./lib/auto-link");
      const result = await autoLinkEdgesCore({ ...input, createdBy: ctx.user?.id ?? null });
      if (!input.dryRun) {
        await logAudit(ctx, "knowledge_edge", "create", 0, {
          autoLink: true, threshold: input.threshold, maxPerNode: input.maxPerNode, created: result.created,
        });
      }
      return result;
    }),
